...
*) `rename(2`) doesn't support cross-device operation (and Chromium ignores this, or used to).
...
260227:
*) Provide a number of `fallocate()(2)` tokens (with glibc version protection estimated from
   the dates the tokens were added to the kernel headers), and expose `MADV_POPULATE_READ`
*) Provide the `mlock2(2)` syscall wrapper.
